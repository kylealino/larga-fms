...
class FMS_DeliveryReports extends BaseController
{
    public function __construct()
    {
        $this->request = \Config\Services::request();
        $this->model = model('App\Models\FMS_DeliveryReports_Model');
    ...
    private function resolveReportData($report_type, $filters)
    {
        switch ($report_type) {
            case 'dr_report':
                return $this->model->getDRReport($filters);
            case 'pod_monitoring':
                return $this->model->getPODMonitoring($filters);
            case 'delivery_performance':
                return $this->model->getDeliveryPerformance($filters);
            case 'partial_failed_delivery':
                return $this->model->getPartialFailedDelivery($filters);
            case 'delivery_report':
            default:
                return $this->model->getDeliveryReport($filters);
        }
    }
    ...
    public function index()
    {
        ...
        switch ($meaction) {
            case 'MAIN':
                return view('fms/reports/delivery-reports-main');
                break;
