...
<style>
    /* ============================================ */
    /* DRIVER MODULE - PROFESSIONAL UX APPROACH */
    /* ============================================ */
    :root {
        --primary: #1a6bb0;
        --primary-dark: #0f5a99;
    ...
    .rating-badge.good { background: #dbeafe; color: #1e40af; }
    .rating-badge.satisfactory { background: #fef3c7; color: #92400e; }
    .rating-badge.needs_improvement { background: #fee2e2; color: #991b1b; }
    .rating-badge.poor { background: #fecaca; color: #7f1d1d; }
...
<div class="lrg-module-header">
    ...
    <div class="header-actions">
        ...
        <button class="btn-header btn-header-primary" onclick="openAddDriver()">
            <i class="bi bi-plus-circle"></i> New Driver
        </button>
    </div>
</div>
...
<div class="stat-grid">
    <div class="stat-card" data-filter="all" onclick="filterTable('all')">
        <div class="stat-left">
            ...
            <div class="stat-sub">All drivers <span class="active-filter-badge" id="filterBadgeAll" style="display:none;">Active Filter</span></div>
        </div>
    ...
    <div class="stat-card" data-filter="ON TRIP" onclick="filterTable('ON TRIP')">
        <div class="stat-left">
            ...
            <div class="stat-sub">Currently on road <span class="active-filter-badge" id="filterBadgeOnTrip" style="display:none;">Active Filter</span></div>
        </div>
    ...
    <div class="stat-card" data-filter="INACTIVE" onclick="filterTable('INACTIVE')">
        <div class="stat-left">
            ...
            <div class="stat-sub">Leave / Suspended <span class="active-filter-badge" id="filterBadgeInactive" style="display:none;">Active Filter</span></div>
        </div>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h6><i class="bi bi-table me-2"></i>Driver Directory 
                    <span id="filterStatusDisplay" style="font-size:11px;font-weight:400;color:var(--gray-500);margin-left:8px;"></span>
                </h6>
                <div>
                    <span class="badge badge-secondary"><?=count($drivers);?> records</span>
            ...
            <div class="card-body">
                <div class="toolbar">
                    <div class="toolbar-left">
                        <button class="btn-toolbar btn-toolbar-primary" onclick="openAddDriver()">
                        ...
                        <tbody>
                            <?php if(count($drivers) > 0): ?>
                                ...
                                <tr>
                                    ...
                                    <td><span class="badge badge-primary"><?=$row['driver_code'];?></span></td>
                                    ...
                                    <td>
                                        ...
                                        if($rating >= 4.5) { $ratingClass = 'excellent'; $ratingLabel = 'Excellent'; }
                                        elseif($rating >= 3.5) { $ratingClass = 'good'; $ratingLabel = 'Good'; }
                                        elseif($rating >= 2.5) { $ratingClass = 'satisfactory'; $ratingLabel = 'Satisfactory'; }
                                        elseif($rating >= 1.5) { $ratingClass = 'needs_improvement'; $ratingLabel = 'Needs Improvement'; }
                                        elseif($rating > 0) { $ratingClass = 'poor'; $ratingLabel = 'Poor'; }
                                        ?>
                                        <?php if($rating > 0): ?>
                                    ...
                                    <td class="text-center">
                                        <div class="action-group">
                                            <button class="btn-icon btn-icon-view" 
                                                    onclick="__Drivers.__viewSkillsets(<?=$row['driver_id'];?>, '<?=addslashes($row['driver_name']);?>')" 
                                                    title="Manage Skills">
                                            ...
                                            <button class="btn-icon btn-icon-delete" 
                                                    onclick="__Drivers.__showDeleteModal(<?=$row['driver_id'];?>, '<?=addslashes($row['driver_name']);?>')" 
                                                    title="Delete Driver">
                ...
                <div class="empty-state">
                    ...
                    <p>Click <strong>New Driver</strong> to add your first driver.</p>
                </div>
                <?php endif; ?>
            </div>
        </div>
...
<!-- ============================================ -->
<div class="modal fade" id="driverModal" tabindex="-1" data-bs-backdrop="static">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="driverModalTitle">
                    <i class="bi bi-person-plus me-2"></i>New Driver
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form id="driverForm" enctype="multipart/form-data">
                <div class="modal-body">
                    <input type="hidden" id="form_driver_id">
                    <input type="hidden" id="form_driver_code">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" id="form_driver_code_display" class="form-control" readonly>
                        </div>
                        <div class="col-md-9">
                            <label class="form-label">Driver Name <span class="required">*</span></label>
                            <input type="text" id="form_driver_name" class="form-control" placeholder="Enter driver name" required>
                        </div>
                    </div>
                    ...
                    <!-- Employment Type + Date Hired + Status -->
                    <div class="row mb-3">
                        <div class="col-md-4">
                    ...
                    <div class="form-section">
                        <div class="form-section-title"><i class="bi bi-clipboard-data"></i> Qualifications & Experience</div>
                        
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <label class="form-label">Years Experience</label>
                                <input type="number" id="form_years_experience" class="form-control" value="0" min="0">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Heavy Vehicle Experience</label>
                                <input type="number" id="form_heavy_vehicle_experience" class="form-control" value="0" min="0">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Tractor Head Experience</label>
                                <input type="number" id="form_tractor_head_experience" class="form-control" value="0" min="0">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">10-Wheeler Experience</label>
                                <input type="number" id="form_ten_wheeler_experience" class="form-control" value="0" min="0">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label class="form-label">Route Experience</label>
                                <input type="text" id="form_route_experience" class="form-control" placeholder="e.g., Luzon Routes">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">Cargo Handling Experience</label>
                                <input type="text" id="form_cargo_handling_experience" class="form-control" placeholder="e.g., Containerized, General Cargo">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label">Training Expiration Date</label>
                                <input type="date" id="form_training_expiration_date" class="form-control">
                            </div>
                        </div>
                        ...
                        <div class="row mb-0">
                            <div class="col-md-12">
                                ...
                                <textarea id="form_other_certifications" class="form-control" rows="2" placeholder="Other certifications"></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Driver Rating -->
                    <div class="form-section">
                        <div class="form-section-title"><i class="bi bi-star"></i> Driver Rating</div>
                        
                        <div class="row mb-0">
                            <div class="col-md-3">
                                <label class="form-label">Overall Rating</label>
                                <input type="number" id="form_overall_rating" class="form-control" value="0" step="0.1" min="0" max="5">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Rating Date</label>
                                <input type="date" id="form_rating_date" class="form-control">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Evaluated By</label>
                                <input type="text" id="form_evaluated_by" class="form-control" placeholder="Evaluator name">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Previous Rating</label>
                                <input type="number" id="form_previous_rating" class="form-control" value="0" step="0.1" min="0" max="5" readonly>
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col-md-12">
                                <label class="form-label">Evaluation Remarks</label>
                                <textarea id="form_evaluation_remarks" class="form-control" rows="2" placeholder="Evaluation remarks"></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        <div class="form-section-title">
                            <i class="bi bi-credit-card"></i> Driver's License
                            <span class="badge badge-secondary" style="font-size:8px;">Optional</span>
                        </div>
                        <p class="text-muted" style="font-size:12px;">Fill out if the driver has a valid license.</p>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="date" id="form_issue_date" class="form-control">
                            </div>
                            <div class="col-md-3">
                    ...
                    <div class="mb-0">
                        ...
                        <textarea id="form_qualification_remarks" class="form-control" rows="2" placeholder="Additional qualification notes"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" id="formSubmitBtn">
                        <i class="bi bi-save"></i> <span id="formBtnText">Save Driver</span>
                    </button>
                </div>
            </form>
        </div>
...
<div class="modal fade" id="viewModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-person-circle me-2"></i>Driver Details</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
...
<!-- SKILLSETS MODAL -->
<!-- ============================================ -->
<div class="modal fade" id="skillsetsModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-star me-2"></i>Driver Skillset Rating</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <input type="hidden" id="sk_driver_id">
                <div class="d-flex align-items-center gap-3 mb-3 p-3" style="background:var(--gray-50);border-radius:8px;">
                    ...
                    <div>
                        <div class="fw-semibold" id="sk_driver_name" style="font-size:15px;"></div>
                        <div class="text-muted" style="font-size:12px;">Rate driver skills (1 - Poor, 5 - Excellent)</div>
                    </div>
                </div>
                ...
                <div class="row g-2 mb-3 p-3" style="background:#ffffff;border:1px solid var(--gray-200);border-radius:8px;">
                    <div class="col-md-3">
                        <select id="sk_skill_name" class="form-control">
                            <option value="">— Select Skill —</option>
                            <option value="Heavy Truck Driving">Heavy Truck Driving</option>
                            <option value="10-Wheeler Operation">10-Wheeler Operation</option>
                            <option value="Tractor Head Operation">Tractor Head Operation</option>
                            <option value="Long-Distance Driving">Long-Distance Driving</option>
                            <option value="City / Urban Driving">City / Urban Driving</option>
                            <option value="Highway Driving">Highway Driving</option>
                            <option value="Maneuvering / Reverse Parking">Maneuvering / Reverse Parking</option>
                            <option value="Defensive Driving">Defensive Driving</option>
                            <option value="Fuel-Efficient Driving">Fuel-Efficient Driving</option>
                            <option value="Vehicle Inspection">Vehicle Inspection</option>
                            <option value="Basic Vehicle Troubleshooting">Basic Vehicle Troubleshooting</option>
                            <option value="Cargo Handling">Cargo Handling</option>
                            <option value="Safety Compliance">Safety Compliance</option>
                            <option value="Route Familiarity">Route Familiarity</option>
                            <option value="Customer Handling">Customer Handling</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <select id="sk_rating" class="form-control">
                    ...
                    <div class="col-md-2">
                        <input type="date" id="sk_rating_date" class="form-control" value="<?=date('Y-m-d');?>">
                    </div>
                    <div class="col-md-3">
                        <input type="text" id="sk_evaluated_by" class="form-control" placeholder="Evaluator">
                    </div>
                    <div class="col-md-2">
                        <button class="btn btn-primary btn-sm w-100" id="skActionBtn" onclick="__Drivers.__saveSkillset()" style="font-size:12px;padding:6px 8px;white-space:nowrap;">
                            <i class="bi bi-plus"></i> Add
                        </button>
                    </div>
                    <div class="col-12 mt-2">
                        <input type="text" id="sk_remarks" class="form-control form-control-sm" placeholder="Remarks (optional)">
                    </div>
                </div>

                <!-- Skillset Table -->
                <div class="table-responsive">
                    <table class="table table-hover" id="skillsetTable">
                        <thead>
                            <tr>
                                <th>Skill</th>
                        ...
                        <tbody id="skillsetBody">
                            <tr>
                                <td colspan="6" class="text-center text-muted">No skills rated yet</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h5 class="mt-3">Delete Driver</h5>
                <p class="text-muted">You are about to delete:<br>
                    <strong id="delete_driver_name" class="text-danger"></strong>
                </p>
...
$(document).ready(function () {
    ...
    $('#driverForm').on('submit', function(e) {
        ...
        if(driver_id) {
            __Drivers.__updateDriverModal();
        } else {
            __Drivers.__saveDriverModal();
        }
    });
});
...
function filterTable(status) {
    ...
    // Status column is now index 4 (0-based: Photo=0, Code=1, Name=2, Contact=3, Status=4)
    if(status === 'all') {
        driverTable.column(4).search('').draw();
        $('.stat-card[data-filter="all"]').addClass('active');
        $('#filterBadgeAll').show();
        $('#filterStatusDisplay').text('(Showing all drivers)');
        $('#clearFilterBtn').show();
    } else if(status === 'AVAILABLE') {
        driverTable.column(4).search('^Available$', true, false).draw();
        ...
        driverTable.column(4).search('On Leave|Suspended|Inactive', true, false).draw();
...
function openAddDriver() {
    $('#driverModalTitle').html('<i class="bi bi-person-plus me-2"></i>New Driver');
    $('#formBtnText').text('Save Driver');
    $('#form_driver_id').val('');
    $('#form_driver_code').val('');
    $('#form_driver_code_display').val('Auto-generated');
    $('#form_driver_name').val('');
    $('#form_contact_number').val('');
    $('#form_address').val('');
    $('#form_employment_type').val('');
    $('#form_date_hired').val('');
    $('#form_driver_status').val('AVAILABLE');
    $('#form_emergency_contact').val('');
    $('#form_emergency_contact_number').val('');
    $('#form_years_experience').val('0');
    ...
    $('#form_overall_rating').val('0');
    $('#form_rating_date').val('');
    $('#form_evaluated_by').val('');
    $('#form_previous_rating').val('0');
    $('#form_evaluation_remarks').val('');
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            if(data) {
                ...
                $('#driverModalTitle').html('<i class="bi bi-pencil me-2"></i>Edit Driver');
                $('#formBtnText').text('Update Driver');
                $('#form_driver_id').val(data.driver_id);
                $('#form_driver_code').val(data.driver_code);
                $('#form_driver_code_display').val(data.driver_code);
                $('#form_driver_name').val(data.driver_name);
                ...
                $('#form_driver_status').val(data.driver_status);
                $('#form_emergency_contact').val(data.emergency_contact);
                $('#form_emergency_contact_number').val(data.emergency_contact_number);
                $('#form_years_experience').val(data.years_experience);
                $('#form_heavy_vehicle_experience').val(data.heavy_vehicle_experience);
                $('#form_tractor_head_experience').val(data.tractor_head_experience);
                $('#form_ten_wheeler_experience').val(data.ten_wheeler_experience);
                $('#form_long_distance_experience').val(data.long_distance_experience);
                $('#form_city_urban_experience').val(data.city_urban_experience);
                $('#form_highway_experience').val(data.highway_experience);
                $('#form_route_experience').val(data.route_experience);
                $('#form_cargo_handling_experience').val(data.cargo_handling_experience);
                $('#form_defensive_driving_training').val(data.defensive_driving_training);
                $('#form_safety_training').val(data.safety_training);
                $('#form_other_certifications').val(data.other_certifications);
                $('#form_training_expiration_date').val(data.training_expiration_date);
                $('#form_qualification_remarks').val(data.qualification_remarks);
                $('#form_overall_rating').val(data.overall_rating);
                $('#form_rating_date').val(data.rating_date);
                $('#form_evaluated_by').val(data.evaluated_by);
                $('#form_previous_rating').val(data.overall_rating);
                $('#form_evaluation_remarks').val(data.evaluation_remarks);
                ...
                $('#form_issue_date').val(data.issue_date);
        ...
        error: function(xhr, status, error) {
            toastr.error("Error loading driver: " + error);
        }
    });
}
                ...
                var html = `
                    <div class="row">
                        <div class="col-md-3 text-center">
                            ...
                            <h5 class="mt-2">${data.driver_name}</h5>
                            <span class="badge badge-primary">${data.driver_code}</span>
                        </div>
                        <div class="col-md-9">
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-2">
                                    ...
                                    <div><strong>${data.years_experience || 0} years</strong></div>
                                </div>
                                <div class="col-md-6 mb-2">
                                    <small class="text-muted">Overall Rating</small>
                                    <div><strong>${data.overall_rating || 0} ★</strong></div>
                                </div>
                                ...
                                <div class="col-md-12 mt-2">
                                    ...
                                    <div class="mt-1">
                                        ...
                                        ${data.issue_date ? '<span class="ms-2"><strong>Issued:</strong> ' + data.issue_date + '</span>' : ''}
...
<script src="<?=base_url('assets/js/apps/fms/driver/driver.js');?>"></script>
